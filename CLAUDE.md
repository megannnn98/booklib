...
3. **Мутирующее и запуск процессов — только с петли.** Сервис умеет слушать сеть
   ...
   `/api/open`, `/api/rescan`, `/api/settings*`, `/api/book` собраны в один
   `priv_routes` APIRouter c зависимостью `require_local` (только `127.0.0.1`/`::1`,
   fail-closed) СВЕРХ `require_own_page`. `BOOKLIB_HOST=0.0.0.0` допустим ровно
   постольку, поскольку это разделение существует и покрыто тестами
   (`tests/integration/test_lan_access.py`). Убрать `require_local` с роутера или
   добавить мутирующую ручку мимо него — регрессия безопасности.
4. **`X-Booklib` обязателен** для привилегированных ручек: `/api/open`, `/api/rescan`,
   `/api/book` и всех трёх `/api/settings` (включая GET-предпросмотр). Локалхост не
