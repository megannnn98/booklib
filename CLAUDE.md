# CLAUDE.md — booklib

Локальный веб-каталог библиотеки — каталога, указанного в настройках (по умолчанию
`~/Books`): превью на каждую книгу, разделы по смыслу, клик открывает папку
в файловом менеджере с выделенным файлом.
Стек: Python + FastAPI + Typer, зависимости через `uv` (`pyproject.toml`), ванильный JS.
Линт и формат — ruff, типы — mypy, тесты — pytest, хуки — pre-commit.

## Железные инварианты

1. **Библиотека строго read-only.** Это активные раздачи qBittorrent (84 торрента):
   переименование или перемещение сломает fastresume-файлы. Никогда не пишем в
   `/run/media/b/DOWNLOADS/books` — ни чистки мусорных `Get More....txt`, ни
   переименований «для порядка». Всё изменяемое живёт в `~/.cache/booklib`.
2. **Диск внешний.** Скан обязан прерваться, если корень не смонтирован или найдено
   0 книг при непустом каталоге (`errors.LibraryUnavailable`). Без этого одна
   перезагрузка без диска пометит все 354 карточки как missing и сотрёт ручную
   раскладку. Guard проверен мутационно — не ослаблять. Смена корня его не ослабляет:
   новый корень стартует с пустой СУБД (`known = 0`), ложного срабатывания нет.
3. **Мутирующее и запуск процессов — только с петли.** Сервис умеет слушать сеть
   (`BOOKLIB_HOST=0.0.0.0` для чтения и скачивания с телефона), поэтому
   `/api/open`, `/api/rescan` и `/api/settings*` собраны в один `priv_routes`
   APIRouter c зависимостью `require_local` (только `127.0.0.1`/`::1`, fail-closed)
   СВЕРХ `require_own_page`. `/api/book` теперь живёт в shared_routes: его можно
   менять с любого адреса, но только через `X-Booklib` и общую серверную БД.
   `BOOKLIB_HOST=0.0.0.0` допустим ровно постольку, поскольку это разделение
   существует и покрыто тестами (`tests/integration/test_lan_access.py`). Убрать
   `require_local` с роутера или добавить мутирующую ручку мимо него — регрессия
   безопасности.
4. **`X-Booklib` обязателен** для привилегированных ручек: `/api/book`, `/api/open`,
   `/api/rescan` и всех трёх `/api/settings` (включая GET-предпросмотр). Локалхост не
   защищает от чужой вкладки: простой cross-origin запрос возможен, а кастомный
   заголовок без CORS-preflight — нет. Preflight мы не обслуживаем. GET'ы настроек
   закрыты тем же заголовком не из-за чтения ответа (его браузер и так не отдаст
   чужому origin), а чтобы не плодить исключений из правила. Новые read-only
   ручки `/api/files` и `/api/download` заголовка НЕ требуют намеренно: иначе
   ссылкой на книгу нельзя поделиться, а файл на 350 МБ пришлось бы тянуть в
   память телефона через fetch+blob, потеряв Range.
5. **Путь книги валидируется** на принадлежность корню библиотеки через `resolve()` +
   `is_relative_to()` → иначе 403.

## Как устроено

```
src/booklib/config/settings.py  настройки: init → config.json → env BOOKLIB_* → .env → умолчания
src/booklib/rootcheck.py        валидация и предпросмотр нового корня
src/booklib/tools.py            REQUIRED_TOOLS (общий для cli и api)
src/booklib/errors.py           LibraryUnavailable — общее исключение обхода и записи
src/booklib/paths.py            library_root / relative_to_root
src/booklib/db.py               connect()/connect_at(): схема, миграции, WAL-ретрай;
                                init_state() — одноразовая подготовка слота на старте
src/booklib/service.py          rescan() и apply_root(): замок скана, общий для api и cli
src/booklib/grouping.py         обход дерева → карточки (BookGroup) → сводка обхода
src/booklib/scanner.py          инкрементальный диф карточек → СУБД
src/booklib/meta.py             нормализация имени файла, название/автор/год
src/booklib/covers.py           превью: pdftocairo / ddjvu+Pillow / zip(epub) / base64(fb2)
src/booklib/taxonomy.py         разделы: overrides → taxonomy.json → rules.json → «Новое»
src/booklib/opener.py           DBus FileManager1.ShowItems, запасной xdg-open
src/booklib/api/app.py          FastAPI + статика в api/static (роуты без orchestration)
src/booklib/cli/app.py          Typer CLI, точка входа `booklib`
scripts/build_taxonomy.py       разовая раскладка → config/taxonomy.json (НЕ в git, см. scripts/README.md)
src/booklib/config/rules.json   правила для новых книг (пакетный дефолт, в git)
config/rules.json               пользовательский оверрайд правил, если нужен
config/taxonomy.json            раскладка (НЕ в git: опись личной библиотеки)
config/taxonomy.example.json    формат + канонический список разделов
tests/{unit,integration}        инварианты из раздела выше
```

Подробнее — `docs/ai-context/architecture.md`.

**Карточка книги** = файлы в одной папке с общим basename после нормализации
(снимаются `.a4`/`.a6` и машинные хвосты `_p117_1`, `_v3`). Одна книга в четырёх
форматах = одна карточка; шесть томов Михайловского = шесть карточек.
Ключ карточки — `относительный_путь_папки/нормализованный_basename`, к нему
привязаны обложка и правки.

**Два слоя правды о разделе:** таблица `books` хранит результат taxonomy, таблица
`overrides` — ручные правки из UI. Выдача склеивает их через `COALESCE`. Поэтому
`taxonomy.json` можно перегенерировать сколько угодно — правки не пострадают.
Не «упрощать» это в одну колонку.

**Состояние скоупится по корню:** `cache_dir/roots/<sha1(root)[:12]>/{library.db,covers}`.
Один и тот же относительный путь в двух библиотеках — это одна строка по PRIMARY KEY
и один файл обложки (`cover_path` = `sha1(key)` без корня), поэтому смешивать корни
в одной СУБД нельзя. Слот даёт и обратную совместимость: возврат на прежний корень
не теряет `overrides`. Корень меняется из UI и живёт в `cache_dir/config.json`,
который перекрывает env — иначе раздел настроек врал бы. Порядок источников:
`init (Settings(...)) → config.json → env → .env → умолчание`; `field_source()`
различает `env` и `env-file`.

**Подготовка состояния — только на старте процесса** (`db.init_state()` из
Typer-callback и lifespan FastAPI): миграция legacy-состояния и `root.txt`.
В `connect()` этого нет намеренно — см. грабли.

**Orchestration живёт в `service.py`**, не в роутах: `rescan()` и
`apply_root(root, scan_on_start)` держат один `_RESCAN_LOCK`, поэтому CLI и HTTP
не расходятся. `cli` не импортирует `api.app` ради скана.

## Грабли, уже собранные (не наступать повторно)

- **`pdftoppm` в poppler 26.07 не пишет jpeg в stdout** — молча даёт 0 байт.
  Используем `pdftocairo -jpeg -singlefile`.
- **`LOWER()` в SQLite умеет только ASCII.** Кириллический поиск без учёта регистра
  требует зарегистрированной питоновской функции `pylower` (см. `db.connect_at()`).
- **`COLLATE NOCASE` в SQLite — ASCII-only, ровно как `LOWER()`.** Сортировка
  «по названию» кидала все кириллические названия со строчной буквы в хвост
  списка (`'Я' = 'я' COLLATE NOCASE` → 0). Лечится `create_collation("unicode_ci", …)`
  рядом с `pylower`, а не сменой СУБД. Регистрация — в `connect_at()`, единственной
  фабрике соединений: пока она жила в обёртке `api.app.db()`, SQL с `unicode_ci`
  был корректен только на api-соединениях, а из CLI падал `no such collation`.
- **Теги тоже завязаны на `unicode_ci` в UNIQUE-колонках.** `tags.name` и
  `tag_aliases.alias` объявлены как `TEXT ... COLLATE unicode_ci UNIQUE`, поэтому
  сторонний `sqlite3 library.db` без регистрации коллации ломается уже на чтении
  схемы/индексов (`no such collation sequence`). Внутри booklib это безопасно
  только потому, что `connect_at()` регистрирует коллацию до `executescript(SCHEMA)`.
- **Правила `rules.json` матчатся ТОЛЬКО по имени файла**, папка игнорируется
  намеренно: слово `embedded` в имени `programming-embedded/` утаскивало все 136
  файлов этой папки в один раздел. Физической раскладке доверять нельзя — там же
  лежит анархистская литература.
- **Порядок источника обложки ≠ порядок primary_file.** Обложка: epub → fb2 → pdf →
  djvu (в epub настоящая обложка, первая страница PDF часто оказывается схемой).
  `primary_file` для nemo: pdf → djvu → epub → fb2 → rtf.
- **`GROUP BY section` неоднозначен** — колонка есть и в `books`, и в `overrides`.
- **Окно nemo называется именем папки**, а не «nemo». Проверять открытие через
  `wmctrl -l | grep <имя папки>`, иначе покажется, что ShowItems не сработал.
- **Клик по карточке с телефона нельзя оставлять на `/api/open`** — это откроет
  nemo на ПК, а не скачает книгу. Клик разводится по роли: локально — nemo
  (`openBook`), удалённо — лист файлов `/api/files` со ссылками на скачивание
  (`openFiles`). Роль берётся из `/api/status.local`.
- **`/api/download` валидирует файл по whitelist'у из `files_json` карточки ПЛЮС
  `resolve()/is_relative_to()` корня** — любой из двух guard'ов в одиночку
  недостаточен (whitelist без resolve не защитил бы от записи `../` в СУБД,
  resolve без whitelist открыл бы любой файл под корнем).
- **`object-fit: cover` калечит альбомные превью** (слайды курсов, даташиты).
  Только `contain` с тёмной подложкой.
- **`pkill -f "<строка>"` убивает сам вызывающий шелл**, если строка встречается в
  его командной строке. Обход: `pkill -f "876[6]"`.
- **В `api_edit` сравнивать входящее поле с базовым значением из `books`, а НЕ с
  `COALESCE`.** Форма шлёт все три поля. Сравнение со склеенным значением затирает
  уже существующую правку другого поля; отсутствие сравнения вообще — копирует
  данные таксономии в `overrides` и замораживает их навсегда.
- **Замена набора тегов удаляет только `source = 'manual'`.** Это задел под
  автоматическую расстановку на следующем этапе; чистить все `book_tags` по `book_key`
  — регрессия, потому что ручная правка не должна сносить чужой источник.
- **`PRAGMA journal_mode=WAL` отдаёт `database is locked` НЕ вызывая busy-handler**,
  поэтому `timeout` соединения на нём не работает. Нужен короткий ретрай; режим
  хранится в файле СУБД, достаточно чтобы его выставило одно соединение.
- **`DecompressionBombError` (Pillow) и `LargeZipFile` наследуют `Exception`
  напрямую**, а не `OSError`/`ValueError`. Генерация обложек ловит широко
  намеренно: одна битая книга не должна ронять рескан каталога.
- **Миграцию legacy-состояния нельзя звать из `connect()`.** Она берёт слот из
  текущих настроек, а `connect()` вызывается уже ПОСЛЕ записи нового корня:
  `booklib config --root /new` сразу после обновления увёз бы `overrides` старого
  корня в слот нового (guard молчит — карточки просто становятся `missing`).
  Место вызова одно — `init_state()` на старте процесса, пока активен корень
  «до переключения».
- **Мутационная проверка требует инвалидации байткода.** Если мутация не меняет
  размер файла (перестановка строк) и восстановление идёт `mv`/`cp` из бэкапа,
  Python считает `.pyc` валидным по `(mtime, size)` и продолжает исполнять
  мутированный код — «исправление не работает» оказывается ложью. Прогонять
  с `PYTHONDONTWRITEBYTECODE=1` и делать `touch` после восстановления.
- **`<form method="dialog">` превращает Enter в закрытие диалога** (implicit
  submission), причём с `returnValue` первой submit-кнопки — то есть «Отмена».
  В настройках это лечится не keydown-хаком, а отказом от `method="dialog"`:
  submit формы = «Проверить», кнопки действий — обычные `button` с обработчиками.
- **pydantic не разворачивает `~` в `Path`-полях из env/.env** — значение
  присваивается дословно, и `BOOKLIB_CACHE_DIR=~/cache` давал каталог с именем
  «~», пока `_config_cache_dir()` (поиск `config.json`) тильду разворачивал:
  запись из UI уходила в один файл, чтение — из другого, выбранный корень «не
  липнул». Лечится `field_validator(mode="before")` на `root`/`cache_dir`/
  `config_dir`, а не `expanduser()` в местах использования.
- **Перенос legacy-состояния идемпотентен по КАЖДОМУ источнику**, а не по факту
  «СУБД уже в слоте»: оборвавшийся на середине перенос (db переехал, covers —
  нет) при раннем выходе по `db_path.exists()` оставлял `covers/` в `cache_dir`
  навсегда и молча. Существующий файл в слоте при этом не затирается.
- **Рантайм-конфиг не валидируется при чтении** — `config.json` читается без
  `validate_root` (иначе повреждённый файл уронил бы старт сервиса). Он может
  содержать путь, который UI/CLI отвергли бы; валидатор — единственный источник
  правды о том, что считается законным корнем.

## Команды

```bash
make help                                 # список целей
make fmt lint typecheck test              # перед коммитом
make restart logs                         # systemd --user сервис

uv run booklib scan --dry-run             # статистика без записи в СУБД
uv run booklib scan --show "Эвола"        # как сгруппировались файлы
uv run booklib covers --report            # книги без обложки и причины
uv run booklib sections                   # применить разделы
uv run booklib classify "путь/файл.pdf"   # куда попадёт новая книга
uv run booklib open --dry-run "<ключ>"    # проверить путь, не открывая nemo
uv run booklib doctor                     # утилиты, библиотека, каталог
uv run booklib config                     # активные настройки и источник каждой
uv run booklib config --root <path>       # сменить корень (валидация + рескан)
```

Всё живёт в одном окружении `uv` (`uv sync --dev`). Системный `python3` больше
не используется — Pillow и остальное приходят из зависимостей проекта.

## Как проверять изменения

- Считать «работает» только после просмотра результата глазами: скриншот витрины
  (`google-chrome-stable --headless --screenshot`) или контактный лист обложек.
  Счётчики API не показывают, что превью — нечитаемый обрезок.
- Для guard'ов делать мутационную проверку: вырезать guard из исходника, прогнать
  на копии СУБД и убедиться, что ущерб реально наступает. Прогон — с
  `PYTHONDONTWRITEBYTECODE=1`, восстановление — с `touch` (см. грабли про `.pyc`). Первая попытка легко
  оказывается пустой — например traversal-путь может просто не существовать, и
  код упадёт по другой причине, ничего не доказав.
- После правки раскладки прогонять `build_taxonomy.py --check` (локальный скрипт) —
  он падает, если появилась карточка без раздела.

## Ориентиры по данным

354 карточки из 506 файлов (273 pdf, 127 djvu + 1 djv, 32 epub, 10 fb2, 8 rtf,
55 mp3). 55 mp3 схлопываются в одну аудио-карточку.
Схлопывание — это реальные дубликаты, а не потеря: `Unity_Assertions_..._v2…v25` —
25 байт-в-байт одинаковых копий. Самые большие разделы: Математика 93,
Даташиты 41, Embedded 29. Раздел «Новое» — карман для неопознанного правилами.
