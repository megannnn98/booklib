...
Теги — это вторая, независимая ось классификации. Полка (`section`) остаётся
основной и не меняется; теги можно назначать вручную и использовать как фильтр.

Гостю доступны чтение и фильтрация:

- `GET /api/tags` — список словаря тегов с алиасами и количеством книг;
- `GET /api/books?tag=...` — фильтр по одному или нескольким тегам;
- поиск `q` тоже учитывает имена тегов и алиасы.

С самого компьютера доступны ручное назначение тегов книгам и управление
словарём: создание, правка, алиасы, объединение, удаление.
...
- `/api/tags` публичен только на чтение; привилегированные ручки для словаря и
  назначения тегов находятся в `priv_routes` и требуют `X-Booklib` + `require_local`;
