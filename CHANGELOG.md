...
- Сохранение карточки стало атомарным: ручные поля и ручные теги теперь
  пишутся одним запросом в одной SQLite-транзакции, а `reset` очищает и
  `overrides`, и `book_tags` со `source = 'manual'`.
- Сохранение карточки с пустыми полями и `tags` снова сбрасывает ручные
  `overrides`, вместо того чтобы оставлять старую правку висеть после замены
  набора тегов.
- Описание тега стало опциональным по смыслу формы: пустая строка, пробелы и
  `null` сохраняются как `NULL`, а отсутствие поля при правке больше не стирает
  текущее описание.
- Алиас больше не может совпадать с каноническим именем тега без учёта
  регистра.
...
### Added
- Вторая ось классификации — теги, независимые от `section`: таблицы `tags`,
  `tag_aliases`, `book_tags` и доменный слой `booklib.tags` (соединение приходит
  параметром, как в `scanner.sync`).
- Чтение тегов доступно и гостю: `GET /api/tags` (с алиасами и числом книг без
  пропавших), `tags` у каждой книги в `GET /api/books`, фильтр
  `GET /api/books?tag=…&tag=…` (пересечение, принимает имя и алиас, складывается
  с разделом, поиском, сортировкой и пагинацией), поиск `q` находит книги по
  именам тегов и алиасам.
- Управление словарём и назначение — в `priv_routes` (`X-Booklib` +
  `require_local`): `POST /api/tags`, `PUT /api/tags/{id}`,
  `POST|DELETE /api/tags/{id}/aliases`, `DELETE /api/tags/{id}` (используемый
  тег → 409 с числом книг), `POST /api/tags/merge` (одна транзакция),
  `PUT /api/book/{key}/tags` (полная замена ручного набора, неизвестное имя →
  404, тег из формы книги не создаётся).
- В витрине: фильтр «🏷 теги» со строкой выбранных чипов, кликабельные плашки на
  карточках, блок тегов в правке карточки с datalist и диалог управления
  словарём (создание, правка, алиасы, объединение, удаление) — только локально.
